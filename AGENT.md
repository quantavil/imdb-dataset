...
- **Removal of title.crew.tsv.gz**: `title.crew` only contains unmapped `nm...` raw IDs. Removed from ingestion to save 79MB download and eliminate raw ID junk.
- **Deterministic Ranking**: Rating/vote ties cause rank flip-flops. Fix: `imdb_id ASC` as deterministic 3rd sort key.
- **Ingest Pruning**: Dropped titles froze. Fix: Temp table `prune_unqualified_titles()` to clean up titles no longer qualifying.
...
- `src/ingest.py`: Zero-disk streaming filter for ratings, basics, and TV episodes.
...
- `data/`: Generated JSON dataset (`titles.json`, ~16.8 MB; `titles.json.gz`, ~4.5 MB, git-ignored, distributed via GitHub Releases & GitHub Pages).
...
2. `uv run imdb-ingest` — Ingests and filters titles and TV episodes.
